...
The plugin compiles that real entry point (theme, custom `@utility`s and all), so class ordering and validation match the Tailwind compiler exactly. Enforced at error, all autofixable: `classnames-order`, `enforces-shorthand` (`pt-2 pb-2` → `py-2`), `enforces-negative-arbitrary-values`, `important-modifier-suffix` (v4's trailing `!`), plus `no-contradicting-classname` (suggestion-fixable). Deliberately **off**: `no-arbitrary-value` (arbitrary values are a documented escape hatch), `no-custom-classname` (CSS-module / third-party / plain-CSS classes are legitimate), and `no-unnecessary-arbitrary-value` — the last is owned by [`@coldsmirk/classcheck`](https://github.com/coldsmirk/canon/tree/main/packages/classcheck), whose `suggestCanonicalClasses` is a strict superset (it also catches `h-[3px]` → `h-0.75` and `bg-[var(--x)]` → `bg-(--x)`); enabling both would report the same finding twice. Class strings are picked up from `class` / `className` attributes and the usual call sites (`cn`, `clsx`, `cva`, `tv`, `twMerge`, …) — plugin defaults, unchanged.
